...
bddSatStates(FormLTLK *form)
{
  switch (form->getOper()) {
    ...
    case LTL_OR: {
      return bddSatStates(form->getSubFormLeft()) + bddSatStates(
               form->getSubFormRight());
      break;
    }
    ...
    case LTL_U: {
      ...
      assert(idx >= 0);
      return bddSatStates(form->getSubFormRight()) + (bddSatStates(
               form->getSubFormLeft()) * (*(paramsLTLwSAN->tabVars))[idx]);
      break;
    }
...
getFairConstr(FormLTLK *form)
{
  ...
  if (oper == LTL_U) {
    FormLTLK *constrForm = new FormLTLK(LTL_OR, new FormLTLK(LTL_NOT,
                                        form->getCopy()), form->getSubFormRight()->getCopy());
...
getFairEG(const BDD &reach)
{
  ...
  while (x != x_p) {
    ...
    for (BDDset::iterator constr = paramsLTLwSAN->fairConstr->begin();
         constr != paramsLTLwSAN->fairConstr->end(); constr++) {
      isect *= ( *(paramsLTLwSAN->prod_trans) * getFairEU(reach,
                 x * (*constr)).SwapVariables(
                   *(paramsLTLwSAN->pv), *(paramsLTLwSAN->pv_succ)
                 )).ExistAbstract(*(paramsLTLwSAN->pv_succ_E));
    }
...
satStates_K(FormLTLK *form, const BDD &reach)
{
  ...
  if (subformOp == LTL_PV) {
    subform_bdd = *(subform->getBDD()) *
                  reach; /* jak mamy zmienną zdaniową, to nie ma co liczyć */
  }
  else if (subformOp == LTL_NOT
           && subform->getSubFormLeft()->getOper() ==
           LTL_PV) { /* wystarczy taki warunek jak form jest zredukowane */
    subform_bdd = !*(subform->getSubFormLeft()->getBDD()) * reach;
  }
  else {
    paramsLTLformUpdate(subform);
    subform_bdd = states_LTL_Ex_withInit(
                    reach); /* BDD ze stanami w których prawdziwe jest form */
  }
...
paramsLTLmemPopLast(void)
{
  ...
  paramsLTLbyForm->erase(paramsLTLbyForm->begin() + paramsLTLbyForm->size() -
                         1);
}
...
{
  for (ParamsLTLbyForm::iterator ii = paramsLTLbyForm->begin();
       ii != paramsLTLbyForm->end(); ii++) {
    paramsLTLcleanup(*ii);
  }
...
{
#ifdef VERBOSE
  cout << "Computing fairness constraints for " << paramsLTLwSAN->form->toStr()
       << endl;
#endif

  if (paramsLTLwSAN->fairConstr != NULL) {
    delete paramsLTLwSAN->fairConstr;
  }

  paramsLTLwSAN->fairConstr = new BDDset;
  getFairConstr(
    paramsLTLwSAN->form); /* dopisać asercję sprawdzającą czy jest zainicjalizowany zbiór!!! */
}
...
compTabTrans(void)
{
  ...
  *(paramsLTLwSAN->allXforms) = paramsLTLwSAN->elemForm->getAllXForm();
  ...
  for (FormLTLKvect::iterator formXg = paramsLTLwSAN->allXforms->begin();
       formXg != paramsLTLwSAN->allXforms->end(); formXg++) {
    BDD sat_Xg = bddSatStates(*formXg);
    BDD sat_g = bddSatStates((*formXg)->getSubFormLeft()).SwapVariables(*
                (paramsLTLwSAN->pv), *(paramsLTLwSAN->pv_succ));
...
#ifdef VERBOSE
    cout << "OK, not loading LTL verification parameters for TOO SIMPLE FORMULA"
         << endl;
#endif
    paramsLTLwSAN = NULL;
  }
...
{
  for (VerTreeList::iterator ii = verTreeList.begin(); ii != verTreeList.end();
       ii++) {
    if ((*ii).states == NULL) {
      last_pick = ii;
      picked = true;
