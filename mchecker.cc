...
checkLTLK(SymAutomataNet &san, FormLTLK *form)
{
...
#ifdef VERBOSE
    cout << "Checking final LTL formula (obtained from LTLK formula): " << final_LTL->toStr() << endl;
#endif
    ...
    {
        cout << "RESULT: formula Ef (where f=" << form->toStr() << ") does not hold" << endl;
        result = false;
    }
...
boundedCheckLTLK(SymAutomataNet &san, FormLTLK *form)
{
    ...
    VerTreeLTLK vtree(form); /* tutaj robi się ParamsLTLwSAN dla każdej podfomuły z vtree */
    ...
    while (!done) /* petla dziala tak dlugo jak wynik nie bedzie sfalsyfikowany */
    {
        ...
        if (!checkLoop || (next * reach != cuddMgr->bddZero()))
        {
            ...
            cout << "Checking final LTL formula (obtained from LTLK formula): " << final_LTL->toStr() << endl;
#endif
            if (finalLTLparams == -1) finalLTLparams = paramsLTLsetup(final_LTL);
        ...
        reach_p = reach;
        if (new_states == cuddMgr->bddZero()) /* are there any undiscovered states? */
...
#ifdef VERBOSE
    cout << "Formula Ef (where f=" << form->toStr() << ") is " << (result ? "true" : "false") << endl;
#endif
    ...
    if (next - reach == cuddMgr->bddZero()) cout << " (with full state space)" << endl;
    else cout << " (with partial state space)" << endl;
...
{
    cerr << "Bounded verification of CTLK formula (with loops checking) " << form->toStr() << endl;
    ...
    cout << "FINAL RESULT: " << form->toStr() << " is " << (result ? "true" : "false") << endl;
...
boundedCheckCTLK(SymAutomataNet &san, FormCTLK *form)
{
    ...
    if (!form->isECTLK())
    {
        FERROR("Formula " << form->toStr() << " is not syntactically an ECTLK formula");
    }
    ...
    cout << "Terminated at k=" << k-1;
    if (reach == reach_p) cout << " (with full state space)" << endl;
    else cout << " (with partial state space)" << endl;
