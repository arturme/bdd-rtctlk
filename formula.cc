...
{
  if (!LTL_COND_1ARG(op)) {
    FERROR("Wrong operator supplied");
  }
...
FormLTLK::reduce(void)
{
  if (oper == LTL_NOT && subForm[0]->getOper() == LTL_NOT) {
    FormLTLK *jump =
      subForm[0]->getSubFormLeft(); /* the formula f after two negations, i.e., ~~f */
...
{
  nonPVcount =
    0; /* inicjalizacje licznika podformuł które nie są zmiennymi zdaniowymi */
}
...
{
  for (FormLTLKstrMap::iterator ii = formSet.begin(); ii != formSet.end();
       ii++) {
    cout << ii->first << " (" << ii->second.idx << ")";

    if (ii->second.idx == -1) {
      cout << " (PV, no id assigned)";
    }
